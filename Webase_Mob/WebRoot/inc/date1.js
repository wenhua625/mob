...
Calendar = {
		...
		function(o, e, d){
			with(Calendar){
				...
				if(window.event){
					...
					//curPosX = 5;
					//curPosY = document.body.scrollTop + event.y+8;
				} else{
					curPosX = e.pageX;
					curPosY = e.pageY;
				}
	...
	loadDate		:
		...
		toString : function(){return ["Calendar v1.0", "author:尹慧华", "email:javasoftware@163.com", "version:1.0"].join("\n");}
	//endregion
}
var	__calendar_html = "<style>";
		__calendar_html += "#Calendar__ {background-color:#eeeeee;width:112 !important;width:109px;position:absolute;display:none}";
		__calendar_html += "#Calendar__ ul{list-style-type:none;margin-left:-28px !important;margin:0 0 0 -20px;}";
		__calendar_html += "#Calendar__ ul li{display:block;width:15px;margin:0px;background-color:#fff;text-align:center;float:left;font:11px Tahoma}";
		...
		__calendar_html += "#Calendar__ #dataGrid__ .today{background:url(images/today.gif) center no-repeat;color:blue;}";
		__calendar_html += "#Calendar__ #dataGrid__ .curDay{background:#dedede url(images/check.gif) right bottom no-repeat;color:blue;}";
