...
function outTopItem(el) {
	if ((el.sub != null) && (el.parentElement.openedSub == el.sub.id)) { //opened
		with(el.style) {
			...
			borderBottom = "0px";
			paddingBottom = "3px";
			background = "url(/images/tileback.gif) #E8F2FF";
		}
	}
	else {
		with (el.style) {
...
function restoreSubItem(el) {
	el.style.background = "url(/images/tileback.gif) #E8F2FF";
...
function float(el, title, width, height) { 
	...
	windowURL = 'about:<html><head><title>Loading fav bar</title></head><body><p style="color: #E8F2FF;">If you see this it probably means that this window has has stopped responding.<br><br>Press CTRL-ALT-DEL and kill this window before it takes the entire Windows with it!</p></html>';
	windowName = "floater";
	...
	str += '.outer, .inner	{border: 1px solid #E8F2FF;}\n';
	str += '</style>\n';
	str += '</head>\n';
