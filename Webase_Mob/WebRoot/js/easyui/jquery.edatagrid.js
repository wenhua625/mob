	...
	function buildGrid(target){
		...
		function focusEditor(field){
			...
			if(s_editor == undefined){
			  //$(target).edatagrid('acceptChanges');
			  return;
			}
			s_editor.target.focus();
			s_editor.target.select();
			
			opts.nextIndex = opts.editIndex+1;
			$(s_editor.target).bind('keyup',function(){
				if (window.event.keyCode == 13)
				 {
				    
				    if (opts.nextIndex == $(target).datagrid('getRows').length) 
				    {
				      opts.nextIndex=0;
				      $(target).datagrid('selectRow',  opts.nextIndex);
				    }else{
				    
				    //alert(opts.nextIndex);
				    $(target).datagrid('selectRow',  opts.nextIndex);
				    }
				}
		    }) ;
